...
describe('TracksService', () => {
  ...
  describe('uploadTrack', () => {
    ...
    beforeEach(() => {
      prisma.genre.findFirst.mockResolvedValue({ id: 1, name: 'Pop', slug: 'pop' });
    ...
    it('should reject when file has no buffer', async () => {
      ...
      await expect(
        service.uploadTrack(USER_ID, dto, file),
      ).rejects.toThrow(BadRequestException);
    });
  ...
  describe('getTrackStatus', () => {
    it('should return status for any visible track', async () => {
      ...
      expect(result).toEqual({ trackId: TRACK_ID, status: TrackStatus.PROCESSING });
    });
    ...
    it('should hide private track from unauthenticated users', async () => {
      ...
      await expect(
        service.getTrackStatus(TRACK_ID, undefined),
      ).rejects.toThrow(NotFoundException);
    });
  });
  ...
  describe('updateTrack', () => {
    ...
    it('should update title for the owner', async () => {
      ...
      const result = await service.updateTrack(TRACK_ID, USER_ID, { title: 'New Title' });
    ...
    it('should resolve a new genre when updating', async () => {
      prisma.genre.findFirst.mockResolvedValue({ id: 2, name: 'Rock', slug: 'rock' });
      prisma.track.update.mockResolvedValue(
        buildTrackRecord({ primaryGenre: { id: 2, name: 'Rock', slug: 'rock' } }),
      );

      const result = await service.updateTrack(TRACK_ID, USER_ID, { genre: 'Rock' });
    ...
    it('should remove genre when empty string is provided', async () => {
      ...
      const result = await service.updateTrack(TRACK_ID, USER_ID, { genre: '' });
  ...
  describe('deleteTrack', () => {
    ...
    it('should soft-delete a track for the owner', async () => {
      prisma.track.findFirst.mockResolvedValue({ id: TRACK_ID, uploaderId: USER_ID });
      prisma.track.update.mockResolvedValue({});
    ...
    it('should allow admin to delete any track', async () => {
      prisma.track.findFirst.mockResolvedValue({ id: TRACK_ID, uploaderId: OTHER_USER_ID });
      prisma.track.update.mockResolvedValue({});
  ...
  describe('changeVisibility', () => {
    it('should change visibility for the owner', async () => {
      ...
      prisma.track.update.mockResolvedValue(
        buildTrackRecord({ visibility: TrackVisibility.PUBLIC, publishedAt: new Date() }),
      );
  ...
  describe('getUserTracks', () => {
    const mockArtist = {
      id: USER_ID,
      profile: { displayName: 'Test Artist', avatarUrl: 'https://example.com/avatar.jpg' },
    };
  ...
  describe('getWaveform', () => {
    it('should return waveform data', async () => {
      ...
      expect(result).toEqual({ trackId: TRACK_ID, waveformData: [0.1, 0.3, 0.5] });
    });
  ...
  describe('handleTranscodingCallback', () => {
    ...
    it('should update track status to FINISHED', async () => {
      ...
      expect(result).toEqual({ trackId: TRACK_ID, status: TrackStatus.FINISHED });
    });
