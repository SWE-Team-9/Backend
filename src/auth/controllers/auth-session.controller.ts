...
export class AuthSessionController {
  ...
  async refresh(@Body() refreshTokenDto: RefreshTokenDto) {
    ...
    const { rawToken: newRefreshToken, tokenHash } =
  ...
  async getActiveSessions(@Request() req: any) {
    ...
    const sessions =
      await this.sessionManagement.getActiveSessionsByUserId(userId);
    return {
      sessions,
  ...
  @ApiResponse({
    status: 200,
    description:
      "Email change request created. Check your current email for verification link.",
  })
  @ApiResponse({
    status: 400,
  ...
  async requestEmailChange(
    ...
    return {
      message:
        "Email change request created. Verification link sent to your current email.",
