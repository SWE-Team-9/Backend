...
export class SubscriptionsController {
  ...
  @ApiQuery({ name: 'returnUrl', required: false, description: 'Backward-compat: returnUrl as query param' })
  @ApiResponse({
    status: 200,
    ...
    schema: {
      example: {
        ...
        portalUrl: 'https://mock-portal.example.com/billing?session=bps_mock_abc123',
        capabilities: {
          canUpdatePaymentMethod: true, canCancel: true, canChangePlan: true, canViewReceipts: true,
          canViewPaymentMethods: true, canAddPaymentMethod: true,
          canRemovePaymentMethod: true, canSetDefaultPaymentMethod: true,
        },
        currentPlanCode: 'PRO',
  ...
  @ApiResponse({ status: 409, description: 'Subscription is not scheduled to cancel.' })
  @ApiResponse({ status: 401, description: 'Not authenticated.' })
  @Post('resume')
  @HttpCode(HttpStatus.OK)
  ...
  @ApiResponse({ status: 409, description: 'No active subscription or already cancelled.' })
  @ApiResponse({ status: 401, description: 'Not authenticated.' })
  @Post('cancel')
  @HttpCode(HttpStatus.OK)
  @UsePipes(new ValidationPipe({ whitelist: true, transform: true }))
