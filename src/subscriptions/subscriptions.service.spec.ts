...
import { FREE_UPLOAD_LIMIT, GRACE_PERIOD_DAYS, PLAN_CONFIG, SubscriptionsService } from './subscriptions.service';
...
function makeActiveSub(
  ...
  return {
    ...
    plan: { tier, uploadLimit, name: 'Pro Monthly', features: { adFree: true, offlineListening: true } },
    ...overrides,
  };
}
...
function makeTrack(overrides: Partial<{
  ...
  return {
    ...
    durationMs: 180000,
    coverArtUrl: null,
    files: overrides.files ?? [{ storageKey: 'tracks/uuid.mp3', fileRole: FileRole.STREAM, fileSizeBytes: BigInt(1024) }],
    uploader: {
      profile: {
...
const mockBillingProvider = {
  ...
  createBillingPortalSession: jest.fn().mockResolvedValue({
    portalSessionId: 'ps_mock', portalUrl: 'https://portal.mock', capabilities: [],
  }),
...
describe('SubscriptionsService', () => {
  ...
  beforeEach(async () => {
    ...
    mockBillingProvider.createBillingPortalSession.mockResolvedValue({
      ...
      capabilities: { canUpdatePaymentMethod: true, canCancel: true, canChangePlan: true, canViewReceipts: true },
    });
  ...
  describe('getPlans', () => {
    it('returns all active plans with perks and uploadLimitDisplay', async () => {
      const rawPlans = [
        { id: 'p1', code: 'free-monthly', name: 'Free', tier: SubscriptionTier.FREE,
          priceCents: 0, billingInterval: 'MONTH', uploadLimit: 3, features: {} },
        { id: 'p2', code: 'pro-monthly', name: 'Pro Monthly', tier: SubscriptionTier.PRO,
          priceCents: 999, billingInterval: 'MONTH', uploadLimit: 100, features: {} },
      ];
    ...
    it('displays \'Unlimited\' for plans with uploadLimit=-1', async () => {
      mockPrisma.subscriptionPlan.findMany.mockResolvedValue([
        { id: 'p1', code: 'go-plus', name: 'Go+', tier: SubscriptionTier.GO_PLUS,
          priceCents: 1999, billingInterval: 'MONTH', uploadLimit: -1, features: {} },
      ]);
  ...
  describe('subscribe', () => {
    ...
    it('starts a free trial for first-time PRO subscribers', async () => {
      ...
      mockPrisma.userSubscription.create.mockResolvedValue({ id: 'trial-sub-id' });
      mockPrisma.billingInvoice.create.mockResolvedValue({ id: 'inv-id' });
      ...
      expect(mockPrisma.billingInvoice.create).toHaveBeenCalledWith(
        expect.objectContaining({
          data: expect.objectContaining({ amountDueCents: plan.priceCents, amountPaidCents: 0 }),
        }),
      );
    ...
    it('does not start trial if a TrialRedemption already exists (charges normally)', async () => {
      ...
      mockPrisma.trialRedemption.findUnique.mockResolvedValueOnce({ id: 'redemption-id' }); // already redeemed
      mockPrisma.userSubscription.create.mockResolvedValue({ id: 'new-sub-id' });
      mockPrisma.billingInvoice.create.mockResolvedValue({ id: 'inv-id' });
      mockPrisma.paymentEvent.create.mockResolvedValue({});
      ...
      expect(mockPrisma.paymentEvent.create).toHaveBeenCalledWith(
        expect.objectContaining({
          data: expect.objectContaining({ eventType: 'invoice.payment_succeeded' }),
        }),
      );
    });
    ...
    it('re-activates a cancelled-but-still-active subscription without charging again', async () => {
      ...
      const existing = { ...makeActiveSub(SubscriptionTier.PRO, 100), cancelAtPeriodEnd: true };
      mockPrisma.subscriptionPlan.findFirst.mockResolvedValue(plan);
      mockPrisma.userSubscription.findFirst.mockResolvedValue(existing); // findActiveSubscription
      mockPrisma.userSubscription.update.mockResolvedValue({});
      mockPrisma.paymentEvent.create.mockResolvedValue({});

      const result = await service.subscribe(USER_ID, { subscriptionType: SubscriptionTypeEnum.PRO, paymentMethodId: 'pm_test' });
  ...
  describe('cancelSubscription', () => {
    ...
    it('sets cancelAtPeriodEnd=true and returns accessUntil', async () => {
      ...
      expect(mockPrisma.paymentEvent.create).toHaveBeenCalledWith(
        expect.objectContaining({
          data: expect.objectContaining({ eventType: 'customer.subscription.updated' }),
        }),
      );
    });
  });
  ...
  describe('handleStripeWebhook', () => {
    ...
    it('marks subscription ACTIVE on invoice.payment_succeeded', async () => {
      const sub = { id: 'sub-uuid-3333', userId: USER_ID, stripeCustomerId: 'cus_mock', stripeSubscriptionId: 'sub_mock_test', plan: { name: 'Pro', tier: SubscriptionTier.PRO } };
      mockPrisma.userSubscription.findFirst.mockResolvedValue(sub);

      await service.handleStripeWebhook(
        makeWebhookBuffer('invoice.payment_succeeded', { invoice: 'in_mock_x', amount_paid: 999, currency: 'usd' }), '',
      );
    ...
    it('is idempotent — skips duplicate events', async () => {
      mockPrisma.userSubscription.findFirst.mockResolvedValue({
        id: 'sub-uuid-3333', userId: USER_ID, stripeCustomerId: 'cus_mock', stripeSubscriptionId: 'sub_mock_test', plan: { name: 'Pro', tier: SubscriptionTier.PRO },
      });
      mockPrisma.paymentEvent.findUnique.mockResolvedValue({ id: 'existing-evt' }); // already exists
  ...
  describe('getOfflineTrack', () => {
    ...
    it('prefers STREAM file over ORIGINAL when both are available', async () => {
      ...
      mockPrisma.userSubscription.findFirst.mockResolvedValue(sub);
      const files = [
        { storageKey: 'tracks/original.wav', fileRole: FileRole.ORIGINAL, fileSizeBytes: BigInt(2048) },
        { storageKey: 'tracks/stream.mp3', fileRole: FileRole.STREAM, fileSizeBytes: BigInt(1024) },
      ];
      mockPrisma.track.findFirst.mockResolvedValue(makeTrack({ files }));
  ...
  describe('getUploadQuota', () => {
    it('falls back to FREE_UPLOAD_LIMIT when no subscription exists', async () => {
      ...
      expect(result).toEqual({ uploadLimit: FREE_UPLOAD_LIMIT, uploadedCount: 1 });
    });
  ...
  describe('PLAN_CONFIG static catalog', () => {
    ...
    it('FREE is ad-supported, no downloads, community support, no trial', () => {
      expect(PLAN_CONFIG.FREE).toMatchObject({
        priceCents: 0, uploadLimit: 3, adsEnabled: true,
        canDownload: false, supportLevel: 'community', trialDays: 0,
      });
    });

    it('PRO is ad-free, allows downloads, priority support, 7-day trial', () => {
      expect(PLAN_CONFIG.PRO).toMatchObject({
        priceCents: 999, uploadLimit: 100, adsEnabled: false,
        canDownload: true, supportLevel: 'priority', trialDays: 7,
      });
    });

    it('GO_PLUS is ad-free, allows downloads, priority support, 30-day trial', () => {
      expect(PLAN_CONFIG.GO_PLUS).toMatchObject({
        priceCents: 1999, uploadLimit: 1000, adsEnabled: false,
        canDownload: true, supportLevel: 'priority', trialDays: 30,
      });
    });
  ...
  describe('checkout()', () => {
    beforeEach(() => {
      mockPrisma.subscriptionPlan.findFirst.mockResolvedValue(makePlan(SubscriptionTier.PRO, 100));
      mockPrisma.userSubscription.findFirst.mockResolvedValue(null);
    });
    ...
    it('throws ForbiddenException(EMAIL_NOT_VERIFIED) for unverified users', async () => {
      mockPrisma.user.findUnique.mockResolvedValue({
        email: 'test@example.com', isVerified: false, profile: null,
      });
      await expect(
        service.checkout(USER_ID, { planCode: PlanCodeEnum.PRO }),
    ...
    it('charges full price on non-trial subscription', async () => {
      mockPrisma.trialRedemption.findUnique.mockResolvedValue({ id: 'prior-redemption' });
    ...
    it('sets amountPaidCents=0 and records trial_started event on trial', async () => {
      ...
      expect(mockPrisma.paymentEvent.create).toHaveBeenCalledWith(
        expect.objectContaining({
          data: expect.objectContaining({ eventType: 'customer.subscription.trial_started' }),
        }),
      );
    });
    ...
    it('includes checkoutSessionId and checkoutUrl in response', async () => {
      const result = await service.checkout(USER_ID, { planCode: PlanCodeEnum.PRO });
    ...
    it('throws ConflictException(SUBSCRIPTION_ALREADY_ACTIVE) on duplicate active same-plan', async () => {
      mockPrisma.userSubscription.findFirst.mockResolvedValue(makeActiveSub(SubscriptionTier.PRO, 100));
    ...
    it('passes userId and planId metadata to createCheckoutSession', async () => {
      ...
      expect(mockBillingProvider.createCheckoutSession).toHaveBeenCalledWith(
        expect.objectContaining({
          metadata: expect.objectContaining({ userId: USER_ID, planId: 'plan-uuid-4444' }),
        }),
      );
    });
  });
  ...
  describe('resumeSubscription()', () => {
    ...
    it('calls billing.resumeSubscription and clears cancelAtPeriodEnd flag', async () => {
      ...
      mockPrisma.userSubscription.findFirst
        ...
        .mockResolvedValueOnce(makeActiveSub(SubscriptionTier.PRO, 100, { cancelAtPeriodEnd: false }));
      mockPrisma.track.count.mockResolvedValue(0);

      await service.resumeSubscription(USER_ID);

      expect(mockBillingProvider.resumeSubscription).toHaveBeenCalledWith(
        expect.objectContaining({ providerSubscriptionId: canceledSub.stripeSubscriptionId }),
      );
      expect(mockPrisma.userSubscription.update).toHaveBeenCalledWith(
        expect.objectContaining({
          data: expect.objectContaining({ cancelAtPeriodEnd: false, canceledAt: null }),
    ...
    it('returns updated subscription state after resuming', async () => {
      mockPrisma.userSubscription.findFirst
        .mockResolvedValueOnce(makeActiveSub(SubscriptionTier.PRO, 100, { cancelAtPeriodEnd: true }))
        .mockResolvedValueOnce(makeActiveSub(SubscriptionTier.PRO, 100, { cancelAtPeriodEnd: false }));
      mockPrisma.track.count.mockResolvedValue(0);
  ...
  describe('changePlan()', () => {
    ...
    it('PRO → GO_PLUS: updates subscription planId to GO_PLUS plan', async () => {
      ...
      await service.changePlan(USER_ID, { planCode: ChangePlanCodeEnum.GO_PLUS });
    ...
    it('FREE downgrade path: 10 tracks with FREE limit=3 hides 7', async () => {
      ...
      const tracks = Array.from({ length: 10 }, (_, i) => ({
        id: `t${i}`, hiddenByPlanLimit: false,
      }));
      mockPrisma.track.findMany.mockResolvedValue(tracks);
      ...
      expect(mockPrisma.track.updateMany).toHaveBeenCalledWith(
        expect.objectContaining({
          where: { id: { in: tracks.slice(FREE_UPLOAD_LIMIT).map((t) => t.id) } },
    ...
    it('returns updated subscription plan after change', async () => {
      ...
      const result = await service.changePlan(USER_ID, { planCode: ChangePlanCodeEnum.GO_PLUS });
  ...
  describe('createBillingPortal()', () => {
    ...
    it('passes returnUrl to billing provider', async () => {
      ...
      await service.createBillingPortal(USER_ID, { returnUrl: 'https://app.example.com/return' });

      expect(mockBillingProvider.createBillingPortalSession).toHaveBeenCalledWith(
        expect.objectContaining({ returnUrl: 'https://app.example.com/return' }),
      );
    });
  });
  ...
  describe('getInvoices()', () => {
    ...
    it('returns mapped invoice list', async () => {
      ...
      mockPrisma.billingInvoice.findMany.mockResolvedValue([
        {
          id: 'inv-1', stripeInvoiceId: 'in_mock_1',
          amountDueCents: 999, amountPaidCents: 999,
          currency: 'USD', status: 'PAID',
          dueAt: new Date('2024-01-15'), paidAt: new Date('2024-01-15'), createdAt: new Date('2024-01-15'),
          subscription: { plan: { name: 'Pro Monthly', tier: SubscriptionTier.PRO } },
        },
      ]);
      ...
      mockPrisma.billingInvoice.findMany.mockResolvedValue([
        {
          ...
          amountDueCents: 999, amountPaidCents: 999,
          currency: 'USD', status: 'PAID',
          dueAt: testDate, paidAt: testDate, createdAt: testDate,
          subscription: { plan: { name: 'Pro Monthly', tier: SubscriptionTier.PRO } },
        },
      ]);
    ...
    it('handles null dueAt and paidAt without crashing', async () => {
      ...
      mockPrisma.billingInvoice.findMany.mockResolvedValue([
        {
          id: 'inv-1', stripeInvoiceId: 'in_mock_1',
          amountDueCents: 999, amountPaidCents: 0,
          currency: 'USD', status: 'OPEN',
          dueAt: null, paidAt: null, createdAt: new Date(),
          subscription: { plan: { name: 'Pro Monthly', tier: SubscriptionTier.PRO } },
        },
      ]);
  ...
  describe('handleStripeWebhook() — extended', () => {
    ...
    beforeEach(() => {
      mockPrisma.userSubscription.findFirst.mockResolvedValue({
        id: 'sub-uuid-3333', userId: USER_ID,
        stripeCustomerId: 'cus_mock', stripeSubscriptionId: 'sub_mock_test',
    ...
    it('customer.subscription.updated syncs trialing status', async () => {
      await service.handleStripeWebhook(
        makeWebhookBuf('customer.subscription.updated', { status: 'trialing', cancel_at_period_end: false }),
        '',
      );

      expect(mockPrisma.userSubscription.update).toHaveBeenCalledWith(
        expect.objectContaining({
          data: expect.objectContaining({ status: SubscriptionStatus.TRIALING }),
        }),
      );
    });
    ...
    it('customer.subscription.trial_will_end sends trial ending email', async () => {
      ...
      await service.handleStripeWebhook(makeWebhookBuf('customer.subscription.trial_will_end'), '');
    ...
    it('invoice.payment_succeeded skips duplicate invoice creation', async () => {
      mockPrisma.billingInvoice.findUnique.mockResolvedValue({ id: 'existing-inv' });

      await service.handleStripeWebhook(
        makeWebhookBuf('invoice.payment_succeeded', { invoice: 'in_dupe', amount_paid: 999 }),
        '',
      );
    ...
    it('customer.subscription.deleted applies FREE limit to user tracks', async () => {
      const tracks = Array.from({ length: 5 }, (_, i) => ({ id: `t${i}`, hiddenByPlanLimit: false }));
      mockPrisma.track.findMany.mockResolvedValue(tracks);
  ...
  describe('getOfflineTrack() — extended', () => {
    it('GO_PLUS users can also download tracks', async () => {
      mockPrisma.userSubscription.findFirst.mockResolvedValue(makeActiveSub(SubscriptionTier.GO_PLUS, 1000));
      mockPrisma.track.findFirst.mockResolvedValue(makeTrack());
    ...
    it('upserts OfflineDownload audit record', async () => {
      ...
      expect(mockPrisma.offlineDownload.upsert).toHaveBeenCalledWith(
        expect.objectContaining({
          where: expect.objectContaining({ userId_deviceId_trackId: expect.any(Object) }),
        }),
      );
    });
  ...
  describe('findActiveSubscription()', () => {
    ...
    it('PAST_DUE subscription within grace period IS returned (user keeps access)', async () => {
      const pastDueSub = makeActiveSub(SubscriptionTier.PRO, 100, { status: SubscriptionStatus.PAST_DUE });
  ...
  describe('getMySubscription() — extended', () => {
    ...
    it('includes latestInvoice when one exists', async () => {
      ...
      mockPrisma.billingInvoice.findFirst.mockResolvedValue({
        id: 'inv-latest', amountPaidCents: 999, currency: 'USD',
