...
import { AccountType, ProfileVisibility, SocialPlatform } from '@prisma/client';
...
function buildPrismaMock() {
  const $transaction = jest.fn().mockImplementation((fn: any) =>
...
function buildStorageMock() {
  return {
    upload: jest.fn().mockResolvedValue({ url: 'https://cdn.example.com/avatar/abc.jpg', key: 'avatar/abc.jpg' }),
...
describe('UsersService', () => {
  ...
  describe('getProfileByHandle', () => {
    ...
    it('returns limited fields for a private profile viewed by a stranger', async () => {
      const profile = buildPrismaProfile({ visibility: ProfileVisibility.PRIVATE });
      prisma.userProfile.findUnique.mockResolvedValue(profile);
      ...
      expect(result).toEqual({
        ...
        display_name: 'Test User',
        avatar_url: null,
        account_type: AccountType.LISTENER,
        is_private: true,
      });
    ...
    it('includes track_count for ARTIST profiles', async () => {
      const profile = buildPrismaProfile({ accountType: AccountType.ARTIST });
      prisma.userProfile.findUnique.mockResolvedValue(profile);
    ...
    it('sets track_count to 0 for LISTENER profiles', async () => {
      const profile = buildPrismaProfile({ accountType: AccountType.LISTENER });
      prisma.userProfile.findUnique.mockResolvedValue(profile);
  ...
  describe('updateProfile', () => {
    ...
    it('updates display_name and bio', async () => {
      await service.updateProfile('user-1', { display_name: 'New Name', bio: 'Bio text' });
    ...
    it('sets visibility to PRIVATE when is_private = true', async () => {
      ...
      expect(prisma.userProfile.update).toHaveBeenCalledWith(
        expect.objectContaining({
          data: { visibility: ProfileVisibility.PRIVATE },
        }),
      );
    });
    ...
    it('resolves genres and performs a transaction when favorite_genres is provided', async () => {
      ...
      await service.updateProfile('user-1', { favorite_genres: ['pop', 'rock'] });

      // Transaction wraps the genre-swap, so deleteMany + createMany must be called
      expect(prisma.userFavoriteGenre.deleteMany).toHaveBeenCalledWith({ where: { userId: 'user-1' } });
    ...
    it('throws BadRequestException for an invalid genre slug', async () => {
      ...
      await expect(
        service.updateProfile('user-1', { favorite_genres: ['pop', 'not-a-genre'] }),
      ).rejects.toThrow(BadRequestException);
    });

    it('throws BadRequestException for a non-HTTPS website URL (SSRF guard)', async () => {
      await expect(
        service.updateProfile('user-1', { website: 'http://internal.corp/admin' }),
      ).rejects.toThrow(BadRequestException);
    });
  });
  ...
  describe('updateExternalLinks', () => {
    it('deletes existing links and inserts the new set', async () => {
      prisma.userSocialLink.findMany.mockResolvedValue([
        { platform: SocialPlatform.TWITTER, url: 'https://twitter.com/user', sortOrder: 0 },
      ]);
      ...
      expect(prisma.userSocialLink.deleteMany).toHaveBeenCalledWith({ where: { userId: 'user-1' } });
      expect(prisma.userSocialLink.createMany).toHaveBeenCalledWith(
        expect.objectContaining({
          data: [{ userId: 'user-1', platform: SocialPlatform.TWITTER, url: 'https://twitter.com/user', sortOrder: 0 }],
        }),
      );
    });
  ...
  describe('uploadProfileImage', () => {
    ...
    beforeEach(() => {
      prisma.userProfile.findUnique.mockResolvedValue(
        buildPrismaProfile({ avatarUrl: 'https://cdn.example.com/avatar/old.jpg' }),
      );
      prisma.userProfile.update.mockResolvedValue(buildPrismaProfile());
    });
    ...
    it('updates coverPhotoUrl when type is cover', async () => {
      prisma.userProfile.findUnique.mockResolvedValue(
        buildPrismaProfile({ coverPhotoUrl: 'https://cdn.example.com/cover/old.jpg' }),
      );
