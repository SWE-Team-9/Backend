-- ============================================================================
-- Migration: create missing billing/reporting tables safely
-- Purpose: bring DB back in sync for runtime queries without breaking on re-run.
-- ============================================================================

-- ----------------------------------------------------------------------------
-- 1) Enums needed by reports/appeals
-- ----------------------------------------------------------------------------
DO $$ BEGIN
...
-- Existing enum in older DBs may miss UNDER_REVIEW
DO $$ BEGIN
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'user_billing_user_id_fkey'
  ) THEN
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'trial_redemptions_user_id_fkey'
  ) THEN
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'appeals_user_id_fkey'
  ) THEN
    ALTER TABLE "appeals"
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'appeals_report_id_fkey'
  ) THEN
    ALTER TABLE "appeals"
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'appeals_resolved_by_fkey'
  ) THEN
    ALTER TABLE "appeals"
...
CREATE INDEX IF NOT EXISTS "user_search_indexable_username_idx"
  ON "user_search_indexable"("username");

DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'user_search_indexable_user_id_fkey'
  ) THEN
